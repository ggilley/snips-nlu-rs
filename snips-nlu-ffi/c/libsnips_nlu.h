...
SNIPS_RESULT snips_nlu_create_from_file(char const* file_path, Opaque** client);

SNIPS_RESULT snips_nlu_create_from_dir(char const* root_dir, Opaque** client);

SNIPS_RESULT snips_nlu_create_from_zip(unsigned char const* zip, unsigned int zip_size, Opaque** client);

SNIPS_RESULT snips_nlu_run_parse(Opaque const* client, char const* input, CIntentParserResult** result);

SNIPS_RESULT snips_nlu_run_parse_into_json(Opaque const* client, char const* input, char** result_json);

SNIPS_RESULT snips_nlu_destroy_string(char* string);

SNIPS_RESULT snips_nlu_destroy_client(Opaque* client);

SNIPS_RESULT snips_nlu_destroy_result(CIntentParserResult* result);

SNIPS_RESULT snips_nlu_get_last_error(char **error);

SNIPS_RESULT snips_nlu_get_model_version(char **version);
